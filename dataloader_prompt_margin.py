...
def img_rotate(img:np.ndarray, angle:float, center=None, scale:float=1.0)->np.ndarray:
    """Rotate image.
    Args:
...
def zoom(x:np.ndarray, zx:float, zy:float, row_axis:int=0, col_axis:int=1):
    zoom_matrix = np.array([[zx, 0, 0],
                            [0, zy, 0],
                            [0, 0, 1]])
    h, w = x.shape[row_axis], x.shape[col_axis]

    matrix = transform_matrix_offset_center(matrix=zoom_matrix, x=h, y=w)
    x = cv2.warpAffine(src=x, M=matrix[:2, :], dsize=(w, h),flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REFLECT, borderValue=(0,0,0),)
    return x



def augmentation(img:np.ndarray,hflip:float,vflip:float,rot90:float,rot:float,zo:float,angle:float,zx:float,zy:float)->np.ndarray:
    if hflip:
...
def preprocess_aug(img_list):
    ...
    zoom_range=(0.5, 1.5)
    zx, zy = np.random.uniform(zoom_range[0], zoom_range[1], size=2)
    aug_img_list=[]
    for img in img_list:
...
def populate_train_list(lowlight_images_path:str,normallight_images_path:str=None,overlight_images_path:str=None):
...
class lowlight_loader(data.Dataset):
    ...
    def __init__(self, lowlight_images_path,normal_light_images_path,semi1_path=None,semi2_path=None):
        self.train_list1,self.train_list2 = populate_train_list(lowlight_images_path,normallight_images_path)
        self.size = 256
        self.neg_path=normal_light_images_path
    ...
    def __getitem__(self, index:int)->(torch.Tensor,torch.Tensor):
        ...
        data_lowlight = data_lowlight.resize(size=(self.size,self.size), reducing_gap=Image.ANTIALIAS)
        ref = ref.resize(size=(self.size,self.size), reducing_gap=Image.ANTIALIAS)
        if self.semi1_path==None:
            img_list=preprocess_aug([data_lowlight,ref])
        elif self.semi2_path==None:
            semi1 = Image.open(data_lowlight_path.replace(self.neg_path,self.semi1_path).replace(__old='.JPG',__new='.png'))
            img_list=preprocess_aug([data_lowlight,semi1,ref])
        else:
            semi1 = Image.open(data_lowlight_path.replace(self.neg_path,self.semi1_path).replace(__old='.JPG',__new='.png'))
            semi2 = Image.open(data_lowlight_path.replace(self.neg_path,self.semi2_path).replace(__old='.JPG',__new='.png'))
